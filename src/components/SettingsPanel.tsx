  ...
  return (
    <div className="settings-page">
      <section className="settings-panel">
        ...
        {labels.map((label, index) => (
          <div className="settings-field" key={index}>
            <label htmlFor={`tab-label-${index}`}>{DEFAULT_TAB_LABELS[index]}の名前</label>
            <input
              id={`tab-label-${index}`}
              type="text"
              value={label}
      ...
      <section className="settings-panel">
        ...
        <p className="settings-description">
          Task2・Task3はステップ数（2〜6）と各ステップの題名を自由に設定できます。ステップの中身はTask1と同じ5段階の自己評価です。ステップ数を減らすと、はみ出したステップの進捗・作業時間は失われます。
        </p>
